@tool
extends Node
class_name WAWPThemeBuilder

@export var build_theme_now: bool = false:
	set(value):
		build_theme_now = false
		if value:
			build_theme()

@export_file("*.tres") var save_path: String = "res://ui/themes/wawp_ui_theme.tres"
...
func build_theme() -> void:
	...
	var error: Error = ResourceSaver.save(theme, save_path)

	if error != OK:
		push_error("Failed to save WAWP theme. Error: %s" % error)
		return

	print("Saved WAWP UI theme to: ", save_path)


func _create_label_variation(theme: Theme, variation_name: StringName, font_size: int, outline_size: int) -> void:
...
func _create_panel_variation(theme: Theme, variation_name: StringName) -> void:
...
func _create_button_variation(theme: Theme, variation_name: StringName) -> void:
...
func _create_card_button_variation(
	...
	var style: StyleBoxFlat = _make_button_style(bg_color, border_color, border_width)
